...
class AntColony:
    def __init__(self, distance_matrix, num_ants, num_iterations, decay_rate, alpha=1.0, beta=2.0):
    ...
    def run(self):
        for iteration in range(self.num_iterations):
            ...
            best_path_index = np.argmin([self.path_length(path) for path in paths])
            if self.path_length(paths[best_path_index]) < self.best_path_length:
                self.best_path = paths[best_path_index]
                self.best_path_length = self.path_length(paths[best_path_index])
    ...
    def construct_paths(self):
        ...
        for ant in range(self.num_ants):
            ...
            while cities_to_visit:
                next_city = self.select_next_city(visited[-1], cities_to_visit)
    ...
    def select_next_city(self, current_city, cities_to_visit):
        pheromone_values = [self.pheromone_matrix[current_city][next_city] ** self.alpha
                            for next_city in cities_to_visit]
        heuristic_values = [1.0 / self.distance_matrix[current_city][next_city] ** self.beta
                            for next_city in cities_to_visit]
        probabilities = np.array(pheromone_values) * np.array(heuristic_values)
        probabilities /= np.sum(probabilities)
        return list(cities_to_visit)[np.random.choice(range(len(cities_to_visit)), p=probabilities)]
